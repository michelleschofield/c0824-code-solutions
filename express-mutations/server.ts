...
app.put('/api/actors/:actorId', async (req, res, next) => {
  try {
    ...
    const { firstName, lastName } = req.body;
    if (!Number.isInteger(+actorId)) {
      throw new ClientError(404, `${actorId} is not an integer`);
    }
    if (!firstName) throw new ClientError(404, 'firstName field is required');
    if (!lastName) throw new ClientError(404, 'lastName field is required');
...
app.delete('/api/actors/:actorId', async (req, res, next) => {
  try {
    const { actorId } = req.params;
    if (!Number.isInteger(+actorId))
      throw new ClientError(400, `${actorId} is not an integer`);
    const sql = `
      delete
        from "actors"
        where "actorId" = $1
